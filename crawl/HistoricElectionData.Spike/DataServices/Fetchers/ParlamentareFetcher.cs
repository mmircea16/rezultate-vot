...
{
    public class ParlamentareFetcher : ElectionFetcher
    {
        public ParlamentareFetcher(Election election) : base(election) { }

        protected override void PopulateTopics()
        {
            Election.Topics.Add(
                DataConstants.SenateTopicKey,
                new Topic
                {
                    Id = DataConstants.SenateTopicKey,
                    Name = $"{Election.Name} - {DataConstants.SenateTopicName}"
                }
            );

            Election.Topics.Add(
                DataConstants.DeputiesTopicKey,
                new Topic
                {
                    Id = DataConstants.DeputiesTopicKey,
        ...
        protected override void PopulateSummary()
        {
            foreach (var (idTopic, topic) in Election.Topics)
            {
                var electionSummariesTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Tara_Sumar&parameter={Election.Id}&parameter={idTopic}"
                );
        ...
        protected override void PopulateGeneralResults()
        {
            foreach (var (idTopic, topic) in Election.Topics)
            {
                var candidatesDataTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Tara_Voturi&parameter={Election.Id}&parameter={idTopic}"
                );
        ...
        protected override void GetCounties()
        {
            var countiesTask = HttpClient.GetStringAsync(
                $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Judet_Lista&parameter={Election.Id}&parameter={DataConstants.SenateTopicKey}"
            );
        ...
        {
            var counties = Election.Topics[DataConstants.SenateTopicKey].Rounds[0].Counties;
        ...
        private void PopulateCountySummary(int idCounty)
        {
            ...
            {
                var countiesTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Judet_Sumar&parameter={Election.Id}&parameter={idCounty}&parameter={idTopic}"
                );
        ...
        private void PopulateCountyResults(int idCounty)
        {
            ...
            {
                var candidatesDataTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Judet_Voturi&parameter={Election.Id}&parameter={idCounty}&parameter={idTopic}"
                );
        ...
        private void GetCountyLocalities(int idCounty)
        {
            var localitiesTask = HttpClient.GetStringAsync(
                $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Localitate_Lista&parameter={Election.Id}&parameter={idCounty}"
            );
        ...
        private void PopulateLocalitySummary(int idCounty, string idLocality)
        {
            ...
            {
                var localitiesTask = HttpClient.GetStringAsync(
                        $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Localitate_Sumar&parameter={Election.Id}&parameter={idLocality}&parameter={idTopic}"
                );
        ...
        private void PopulateLocalityResults(int idCounty, string idLocality)
        {
            ...
            {
                var candidatesDataTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Localitate_Voturi&parameter={Election.Id}&parameter={idLocality}&parameter={idTopic}"
                );
        ...
        private void GetLocalityPollingSections(int idCounty, string idLocality)
        {
            var pollingSectionsTask = HttpClient.GetStringAsync(
                $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Sectie_Lista&parameter={Election.Id}&parameter={idLocality}"
            );
        ...
        private void PopulatePollingSectionSummary(int idCounty, string idLocality, int idPollingSection)
        {
            foreach (var (idTopic, topic) in Election.Topics)
            {
                var summaryKey = topic.Id.Equals(DataConstants.SenateTopicKey) ? DataConstants.SenateSummaryKey : DataConstants.DeputiesSummaryKey;

                var pollingSectionsTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Sectie_Sumar&parameter={Election.Id}&parameter={idCounty}&parameter={idPollingSection}&parameter={idTopic}&parameter={summaryKey}"
                );
        ...
        private void PopulatePollingSectionResults(int idCounty, string idLocality, int idPollingSection)
        {
            ...
            {
                var candidatesDataTask = HttpClient.GetStringAsync(
                    $"http://alegeri.roaep.ro/wp-content/plugins/aep/aep_data.php?name=v1_parl_Sectie_Voturi&parameter={Election.Id}&parameter={idCounty}&parameter={idPollingSection}&parameter={idTopic}"
                );
